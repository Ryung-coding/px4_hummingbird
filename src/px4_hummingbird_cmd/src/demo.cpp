...
public:
  Demo() : Node("demo")
  {
    ...
    timeout_timer_ = this->create_wall_timer(std::chrono::microseconds(params::SERVO_PERIOD_US), std::bind(&Demo::timeout_callback, this));
  ...
  bool torque_off_all()
{
  ...
  void odometry_callback(const px4_msgs::msg::VehicleOdometry::SharedPtr msg)
  {
    ...
    utils::TargetCMD target;

    if (!lie_group_rotation_.update(quaternion, target))
    {
      RCLCPP_WARN_THROTTLE(
        this->get_logger(),
        *this->get_clock(),
        1000,
        "Invalid VehicleOdometry quaternion"
      );
      return;
    }
    ...
    for (std::size_t i = 0; i < params::DXL_SERVOS.size(); ++i)
    {
      ...
      goal_ppr[i] = std::clamp(
        static_cast<int32_t>(
          std::lround(
            servo.zero_ppr +
            servo.direction * rad * params::PPR_PER_RAD)),
        servo.min_ppr,
        servo.max_ppr
      );
    }
    ...
    if (!write_goal(goal_ppr))
    {
      ...
      RCLCPP_ERROR(this->get_logger(), "Odometry command write failed. Sending zero.");
      send_zero(params::FINAL_ZERO_REPEAT_COUNT, params::FINAL_ZERO_REPEAT_INTERVAL_MS);
  ...
  {
    if (emergency_active_ || !odometry_received_ || timeout_zero_sent_) return;

    const double timeout = (this->now() - last_odometry_time_).seconds();

    if (timeout < params::SERVO_TIMEOUT_SEC) return;

    RCLCPP_WARN(this->get_logger(), "Odometry timeout: %.3f sec", timeout);
...
int main(int argc, char ** argv)
{
  ...
  try
  {
    auto node = std::make_shared<Demo>();
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("demo"), "Node failed: %s", e.what());
  }
