...
private:
  ...
  void wait_offboard(uint64_t timestamp_us, double elapsed_sec)
  {
    ...
    if (!offboard_mode_active) 
    {
      ...
      if (!has_vehicle_status_ && auto_offboard_ && offboard_request_count_ >= 3)
      {
        RCLCPP_WARN(this->get_logger(), "vehicle_status unavailable. Continuing to arm after repeated offboard commands.");
  ...
  void start_RC2DDS_handoff(double elapsed_sec)
  {
  ...
  {
    const bool vehicle_armed = has_vehicle_status_ && vehicle_status_.arming_state == px4_msgs::msg::VehicleStatus::ARMING_STATE_ARMED;
    ...
    if (!vehicle_armed) 
    {
      ...
      if (!has_vehicle_status_ && auto_arm_ && arm_request_count_ >= 3)
      {
        start_RC2DDS_handoff(elapsed_sec);
        RCLCPP_WARN(this->get_logger(), "vehicle_status unavailable. Starting initial pose handoff after repeated arm commands.");
      }
  ...
  void on_tick()
  {
    ...
    const bool vehicle_status_available = has_vehicle_status_;
    ...
    if (control_mode_ == ControlMode::RC2DDS_handoff)
    {
    ...
    if (!vehicle_status_available)
    {
      ...
      if (control_mode_ != ControlMode::path_active)
      {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "Waiting for vehicle_status; sending auto commands to trigger status update.");
      }
    }

    const bool offboard_mode_active = vehicle_status_available && vehicle_status_.nav_state == px4_msgs::msg::VehicleStatus::NAVIGATION_STATE_OFFBOARD;

    const bool vehicle_armed = vehicle_status_available && vehicle_status_.arming_state == px4_msgs::msg::VehicleStatus::ARMING_STATE_ARMED;

    if (vehicle_status_available && control_mode_ == ControlMode::path_active && (!offboard_mode_active || !vehicle_armed))
    {
