...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        Node(package="px4_hummingbird_cmd", executable="px4_position_cmd", name="px4_position_cmd", output="screen", condition=IfCondition(enable_cmd), parameters=[{"path": path, "auto_offboard": ParameterValue(auto_offboard, value_type=bool), "auto_arm": ParameterValue(auto_arm, value_type=bool)}]),
        Node(package="px4_hummingbird_cmd", executable="px4_servo_to_dynamixel", name="px4_servo_to_dynamixel", output="screen", condition=IfCondition(enable_dynamixel_servo)),
        Node(package="px4_hummingbird_cmd", executable="px4_multirotor_viewer.py", name="px4_multirotor_viewer", output="screen", condition=IfCondition(enable_viewer), parameters=[{
            "viewer_mode": viewer_mode,
            "log_enabled": ParameterValue(viewer_log_enabled, value_type=bool),
            "log_dir": _cmd_str("viewer_log_dir"),
            "log_filename": _cmd_str("viewer_log_filename"),
            **viewer_config,
